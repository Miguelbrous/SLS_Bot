...
- **Cerebro ingest (cron)**: existe `scripts/cron/cerebro_ingest.sh` con variables `CEREBRO_INGEST_*` para lanzar la ingesta desde cron/systemd y guardar automáticamente el snapshot en `tmp_logs/cerebro_ingestion.json`.
