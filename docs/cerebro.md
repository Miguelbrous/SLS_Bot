...
- Útil para cron/CI: `python scripts/ops.py cerebro autopilot --mode test --dataset logs/cerebro_experience.jsonl --min-rows 500 --backfill-rows 600 --output-dir models/cerebro/test/autopilot --max-dataset-age-minutes 60 --require-promote` bloquea datasets viejos y falla cuando el entrenamiento no promueve un nuevo `active_model.json`. El comando ejecuta automáticamente una validación del dataset (`--dataset-min-win-rate`, `--dataset-max-win-rate`, `--dataset-min-symbols`, `--dataset-max-age-hours`) antes de entrenar; usa `--skip-dataset-check` solo si estás en un entorno controlado.
- Añade `--summary-json tmp_logs/cerebro_autopilot_summary.jsonl --summary-append` para guardar cada corrida (JSON/JSONL) con métricas y dataset stats; los textos de Slack y el `.prom` incluyen ahora win rate y edad del dataset.
...
- `scripts/cron/cerebro_autopilot.sh` envuelve el comando para cron: además de `CEREBRO_AUTO_PROM_FILE`, `CEREBRO_AUTO_SLACK_WEBHOOK`, `CEREBRO_AUTO_SLACK_USER`, `CEREBRO_AUTO_REQUIRE_PROMOTE` y `CEREBRO_AUTO_MAX_DATASET_AGE_MIN`, ahora soporta `CEREBRO_AUTO_DATASET_MIN_WIN_RATE`, `CEREBRO_AUTO_DATASET_MAX_WIN_RATE`, `CEREBRO_AUTO_DATASET_MIN_SYMBOLS`, `CEREBRO_AUTO_DATASET_MAX_AGE_HOURS`, `CEREBRO_AUTO_SKIP_DATASET_CHECK`, `CEREBRO_AUTO_SUMMARY_FILE` y `CEREBRO_AUTO_SUMMARY_APPEND` para controlar la validación del dataset y la persistencia de los resúmenes antes de entrenar. Deja logs en `tmp_logs/cerebro_autopilot_runner.log` + `tmp_logs/cerebro_autopilot_<mode>.log`.
